...
Install Blueprint with `npx skills add owainlewis/blueprint`. Use `/architecture`, `/design`, `/architecture-review`, `/plan`, `/test`, `/review`, and `/improve` for one kind of engineering work. Use `/task-to-pr` to deliver one code change and `/milestone` to deliver every issue in a GitHub milestone.
