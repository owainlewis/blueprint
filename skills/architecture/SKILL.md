...
3. Identify the load-bearing facts: the system boundary, source of truth, main parts, dependency direction, important protocols or data, and the one rule a contributor must not break.
...
- include authentication, validation, persistence, side effects, response timing, cleanup, and recovery when they matter;
...
3. **Boundaries.** Does each important part say what it owns and does not own?
