...
3. **Run `implement`** with the ticket as the task. The acceptance criteria are the definition of done. Keep changes narrow. Add or update tests with the code. Update docs or generated-doc sources when behavior, configuration, public APIs, or user-visible commands change.
...
5. **Run `review`** with a fresh subagent. Ask it to inspect the current diff for correctness, regressions, security, maintainability, robustness, and missing tests. Judge every finding, fix the valid in-scope blockers, and re-verify. If no subagent capability exists, self-review and say so in the PR.
